...
public class DataSystem {
    ...
    //Tao nguoi dung moi; bao gom ten dang nhap, mat khau va so tien trong tai khoan
    ...
    public static boolean checkUsername(String username){
        ...
        final File folder = new File("./data");
        for(File file : folder.listFiles()){
            if(file.getName().equals(username)) return true;
        }
        return false;
    }
        ...
        for(File file : folder.listFiles()){
            if(file.getName().equals(username)){
                try{
                    ...
                    if(sc.nextLine().equals(Password)) check=true;
                    else check=false;
                    sc.close();
                }
                catch(Exception e){
